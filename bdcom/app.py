...
#############################################
# 7) Main App with Field Name Filter
#############################################
...
def main():
    ...
    if st.sidebar.button("Generate Report"):
        ...
        st.session_state.value_dist_df = val_dist_df
        st.session_state.pop_comp_df = pop_comp_df
    ...
    # Once data is loaded, add a global filter for Field Name based on the Summary sheet.
    if "summary_df" in st.session_state and not st.session_state.summary_df.empty:
        summary_fields = st.session_state.summary_df["Field Name"].unique().tolist()
        selected_field = st.selectbox("Filter by Field Name (applies to Summary, Value Dist & Pop Comp)", summary_fields, key="global_field_filter")
        st.session_state.active_field = selected_field
    ...
    if "df_data" in st.session_state:
        ...
        mask_sel = (old_pdf["Field Name"] == st.session_state.active_field)
        st.session_state.pop_comp_df = pd.concat([old_pdf[~mask_sel], updated_pop], ignore_index=True)

        #############################################
        # 3) Summary Aggregation and Append Previous Month Comment
        #############################################
        ...
        # Use a plain JavaScript function string for the cell renderer.

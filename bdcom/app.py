...
def generate_summary_df(df_data, date1, date2):
    ...
    # Add editable comment columns (to be updated from detail grids)
    summary["Comment"] = ""
...
def update_summary_comments(summary_df, val_df, pop_df):
    # Aggregate comments from detail grids by Field Name.
    if "Comment" in val_df.columns:
        comments_val = val_df.groupby("Field Name")["Comment"].apply(lambda x: "\n".join(x[x.astype(str).str.strip() != ""])).to_dict()
    else:
        comments_val = {}
    if "Comment" in pop_df.columns:
        comments_pop = pop_df.groupby("Field Name")["Comment"].apply(lambda x: "\n".join(x[x.astype(str).str.strip() != ""])).to_dict()
    else:
        comments_pop = {}
    def combine_comments(field):
        parts = []
        if field in comments_val and comments_val[field]:
            parts.append("Value Distribution: " + comments_val[field])
        if field in comments_pop and comments_pop[field]:
            parts.append("Population Comparison: " + comments_pop[field])
        return "\n".join(parts)
    summary_df["Comment"] = summary_df["Field Name"].apply(combine_comments)
    return summary_df
...
def main():
    ...
    if st.sidebar.button("Generate Report"):
        ...
        # Store the complete (full) flattened data separately so filtering does not affect saved data.
        ...
        # Ensure a Comment column exists in the detail grids
    ...
    if "df_data" in st.session_state:
        ...
        # Dropdown to select field (only one field’s data is shown at a time)
        ...
        # Use the full stored data and filter by the selected field
        ...
        # Disable built-in filtering so the full set is always shown
        gb_val = GridOptionsBuilder.from_dataframe(filtered_val)
        gb_val.configure_default_column(editable=True, filter=False,
        ...
        # Make the Comment column editable (if it exists; if not, add it)
        if "Comment" not in filtered_val.columns:
            filtered_val["Comment"] = ""
        gb_val.configure_column("Comment", editable=True, width=250)
        ...
        # Update the full data for the selected field with any edits (especially in Comment)
        updated_val = pd.DataFrame(val_res["data"]).copy()
        st.session_state.value_dist_df_full.loc[st.session_state.value_dist_df_full["Field Name"] == selected_val_field, "Comment"] = updated_val["Comment"].tolist()
        
        # SQL Logic for Value Distribution
        st.subheader("View SQL Logic (Value Distribution)")
        # Dropdown for SQL logic (separate but defaults to the grid selection)
        ...
        gb_pop.configure_default_column(editable=True, filter=False,
                                        cellStyle={'white-space':'normal','line-height':'1.2em','width':150})
        ...
        gb_pop.configure_column("Comment", editable=True, width=250)
        ...
        st.session_state.pop_comp_df_full.loc[st.session_state.pop_comp_df_full["Field Name"] == selected_pop_field, "Comment"] = updated_pop["Comment"].tolist()
        ...
        ##############################
        # Before Saving: Update Summary Comments from Detail Grids
        ##############################
        st.session_state.summary_df = update_summary_comments(st.session_state.summary_df, 
                                                              st.session_state.value_dist_df_full,
                                                              st.session_state.pop_comp_df_full)
        try:

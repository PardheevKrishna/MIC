...
def drop_prev_comments_columns(df):
    """Drop any columns that contain 'm- Comments' so duplicates are not accumulated."""
...
def flatten_dataframe(df):
    """Flatten a MultiIndex column DataFrame into single-level columns."""
...
#############################################
# 4) Cache and Pivot Previous Comments
#############################################
    ...
    if "df_data" in st.session_state:
        ...
        try:
            with pd.ExcelWriter(st.session_state.input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                ...
                # Optionally, add cell comments in Summary sheet

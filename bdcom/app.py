...
def drop_prev_comments_columns(df):
    # Drop any columns containing "m- Comments"
...
#############################################
# Generate Summary & Distribution Sheets
#############################################
...
#############################################
# 4) Cache Previous Comments & Pivot
#############################################
...
#############################################
# 5) Append Previous Month Comment into Summary
#############################################
...
def append_prev_comment(summary_df, target_month):
    ...
    # Clean up any leftover "nan" substrings
...
def main():
    ...
    if "df_data" in st.session_state:
        ...
        # Append the previous month’s comment into Summary "Comment" column automatically.
        ...
        # Optionally, aggregate additional current grid comments into Summary here (skipping empty or 'nan')
        ...
        # Use a plain JavaScript function string for the cell renderer (avoids JSON serialization issues)
        ...
        try:
            with pd.ExcelWriter(st.session_state.input_file_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                ...
                # Optionally, store Summary "Comment" as Excel cell comments.

...
def wide(df_src):
    ...
    denom = src.groupby(["field_name","filemonth_dt"], as_index=False)\
...
def pivot_comments(df, prefix):
    pts = df.groupby(["field_name","month"])["comment"]\
            .agg(lambda x: "\n".join(x)).reset_index()
...
prev_summary = pd.merge(cur, prev_wide, left_on="Field Name",
                        right_on="field_name", how="left").drop(columns=["field_name"])
...
app.layout = html.Div([
    ...
    # Dynamic title
    html.H2(id="dashboard-title", className="text-center mb-4"),

    # gating controls
    html.Div(className="row mb-4", children=[
        html.Div(className="col-md-4", children=[
            html.Label("Select Portfolio:"), dcc.Dropdown(
                id="select-portfolio",
        ...
        html.Div(className="col-md-4", children=[
            html.Label("Select Excel Report:"), dcc.Dropdown(
                id="select-report",
        ...
        html.Div(className="col-md-4", children=[
            html.Label("Select Month:"), dcc.Dropdown(
                id="select-month",
        ...
        dcc.Tabs(id="main-tabs", children=[
            ...
            dcc.Tab(label="Summary", className="p-3", children=[
                html.Div(className="row mb-4", children=[
                    *[
                        html.Div(className="col-md-3", children=[
                            html.Label(lbl), dcc.Dropdown(
                                id=did,
                                options=[{"label":i,"value":i} for i in sorted(initial_summary[col].unique())],
                                multi=True,
                                value=sorted(initial_summary[col].unique()),
                ...
                dash_table.DataTable(
                    id="summary-table",
                    columns=[
                        {"name":c,"id":c,"editable": True if c in ["Comment Missing","Comment M2M"] else False}
            ...
            dcc.Tab(label="Value Distribution", className="p-3", children=[
                ...
                html.Div(className="mt-3", children=[
                    ...
                    html.Div(id="vd_sql", style={"whiteSpace":"pre-wrap","border":"1px solid #ced4da","padding":"0.5rem","borderRadius":"0.25rem"}),
            ...
            dcc.Tab(label="Population Comparison", className="p-3", children=[
                ...
                html.Div(className="mt-3", children=[
                    ...
                    html.Div(id="pc_sql", style={"whiteSpace":"pre-wrap","border":"1px solid #ced4da","padding":"0.5rem","borderRadius":"0.25rem"}),
            ...
            dcc.Tab(label="Comments", className="p-3", children=[
                ...
                dash_table.DataTable(
                    ...
                    page_size=20,
                    style_table={"overflowX":"auto"},
                    style_cell_conditional=style_cond_prev,
            ...
            dcc.Tab(label="SAS History", className="p-3", children=[
                ...
                html.Div(className="row my-3", children=[
                    ...
                    html.Div(className="col-md-6", children=[
                        html.Label("Threshold (%):"), dcc.Input(
                            id="sas-threshold", type="number", min=0, max=100, step=1, value=50,
                            className="form-control"
                        )
                    ])
                ]),
            ...
            dcc.Tab(label="SAS Ad-hoc", className="p-3", children=[
                ...
                dcc.Textarea(
                    id="sas-code-input", style={"width":"100%","height":"300px"},
                    placeholder="Paste SAS code..."
                ),
                html.Button("Run SAS", id="run-sas-btn", className="btn btn-primary mt-2"),
                html.Div(className="mt-4", children=[
                    html.H5("Log Output:"),
                    html.Pre(id="sas-log-output", style={"whiteSpace":"pre-wrap","border":"1px solid #ccc","padding":"10px"})
                ]),
                html.Div(className="mt-4", children=[
                    html.H5("Data Output:"),
                    dash_table.DataTable(
...
# Show dashboard only after selections
@app.callback(
...
# Update title based on selected portfolio
@app.callback(
    Output("dashboard-title","children"),
    Input("select-portfolio","value")
)
def update_title(portfolio):
    return f"{portfolio or ''} FRY14M Field Analysis"
...
# Update selected value label displays
@app.callback(Output("vd-val-lbl","value"), Input("vd-table","active_cell"), State("vd-table","data"))
def update_vd_label(a, r): return r[a["row"]]["value_label"] if a else ""
@app.callback(Output("pc-val-lbl","value"), Input("pc-table","active_cell"), State("pc-table","data"))
def update_pc_label(a, r): return r[a["row"]]["value_label"] if a else ""

# Previous comments view
@app.callback(
...
def update_prev(sel, show, rows):
    ...
    if trig=="prev_show_all_btn":
        filtered = prev_display
    elif sel:
        fld=rows[sel[0]]["Field Name"]; filtered=prev_display[prev_display["Field Name"]==fld]
    else:
        filtered=prev_display
    return filtered.to_dict("records")

# SAS History outputs per field
@app.callback(
...
def filter_sas_history(fields, threshold):
    ...
    if fields: df=df[df.field_name.isin(fields)]
    ...
    for fld in df.field_name.unique():
        ...
        results.append(dash_table.DataTable(
            ...
            data=out.to_dict("records"), page_size=5,
            style_table={"overflowX":"auto"}, style_cell={"textAlign":"left"}
        ))
    return results

# SAS Ad-hoc placeholder execution
@app.callback(
...
def run_sas_code(n, code):
    ...
    sample = np.random.choice(FIELD_NAMES, size=min(5,len(FIELD_NAMES)), replace=False)
    df = pd.DataFrame({"field_name":sample, "pct":np.round(np.random.uniform(0,100,len(sample)),2)})

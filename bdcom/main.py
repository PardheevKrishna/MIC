...
@app.callback(
    ...
    Input('summary-table','active_cell'),State('summary-table','data')
)
def update_detail(active_cell,summary_rows):
    if active_cell:
        fld = summary_rows[active_cell['row']]['Field Name']
...
def update_comments(n_vd,n_pc,vd_act,vd_data,vd_txt,pc_act,pc_data,pc_txt,summary_data):
    df_sum=pd.DataFrame(summary_data);ctx=dash.callback_context
    if not ctx.triggered: return summary_data
    trig=ctx.triggered[0]['prop_id'].split('.')[0]
    if trig=='vd_comm_btn' and vd_act and vd_txt:
        r=vd_act['row'];fld=vd_data[r]['field_name'];lbl=vd_data[r]['value_label']
        ent=f"{lbl} - {vd_txt}";m=df_sum['Field Name']==fld;old=df_sum.loc[m,'Comment Missing'].iloc[0]
    ...
    if trig=='pc_comm_btn' and pc_act and pc_txt:
        ...
        ent=f"{lbl} - {pc_txt}";m=df_sum['Field Name']==fld;old=df_sum.loc[m,'Comment M2M'].iloc[0]

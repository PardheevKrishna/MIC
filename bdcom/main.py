...
====================
Dash app with Dash-AG-Grid “value-list” (set) filters.

Features
--------
1. Summary grid with two editable comment columns.
2. Single-click a Summary row → detail grids filter to that field
   and SQL logic is shown.
3. Comment boxes in each detail tab append notes to the matching
   comment column in Summary.
4. AG Grid set-filters, sorting, pagination.
...
# 0.  Workbook path
# ────────────────────────────────────────────────────────────────
INPUT_FILE = "input.xlsx"        # adjust if needed
...
# ────────────────────────────────────────────────────────────────
# 3.  Helper to flag pop-comp rows
# ────────────────────────────────────────────────────────────────
_PHRASES = [
...
_contains_phrase = lambda x: any(re.search(p, str(x)) for p in _PHRASES)

# ────────────────────────────────────────────────────────────────
# 4.  Build Summary dataframe (incl. comment cols)
# ────────────────────────────────────────────────────────────────
summary_rows = []
for fld in sorted(df_data["field_name"].unique()):
    ...
    diff1 = df_data[(df_data["analysis_type"] == "pop_comp") &
                    (df_data["field_name"] == fld) &
                    (df_data["filemonth_dt"] == DATE1) &
                    (df_data["value_label"].apply(_contains_phrase))
                    ]["value_records"].sum()
...
# ────────────────────────────────────────────────────────────────
# 5.  Detail dataframes (only the two months)
# ────────────────────────────────────────────────────────────────
mask_two = df_data["filemonth_dt"].isin([DATE1, DATE2])

vd_all = df_data[mask_two & (df_data["analysis_type"] == "value_dist")].copy()
pc_all = df_data[mask_two & (df_data["analysis_type"] == "pop_comp")
                 ].loc[lambda d: d["value_label"]
                       .apply(_contains_phrase)].copy()

vd_all["filemonth_dt"] = vd_all["filemonth_dt"].dt.strftime("%m/%d/%Y")
pc_all["filemonth_dt"] = pc_all["filemonth_dt"].dt.strftime("%m/%d/%Y")

# ────────────────────────────────────────────────────────────────
# 6.  Column-def factory  (AG Grid)
# ────────────────────────────────────────────────────────────────
def make_col_defs(df, hide_sql=False):
    """Return columnDefs with set filter + editability for comments."""
    col_defs = []
    for col in df.columns:
        if hide_sql and col == "value_sql_logic":
            continue
        col_defs.append({
            "headerName": col,
            "field":      col,
            "filter":     "agSetColumnFilter",   # value-list filter
            "floatingFilter": True,
            "sortable":   True,
            "editable":   col.startswith("Comment"),
            "resizable":  True,
        })
    return col_defs

# ────────────────────────────────────────────────────────────────
# 7.  Tiny helper for comment area
# ────────────────────────────────────────────────────────────────
def comment_box(text_id, btn_id):
    return html.Div([
        dcc.Textarea(id=text_id,
...
#  grid options reused for all three grids
GRID_OPTS = {
    "pagination": True,
...
@app.callback(
    ...
    Input("summary", "cellClicked"),          # click in Summary grid
...
def master_cb(click_evt, n_vd, n_pc,
    ...
    # ── A. Append comments from text areas ─────────────────
    ...
    # ── B. Determine active field (row click) ──────────────
    if trig == "summary" and click_evt and "rowIndex" in click_evt:
        row_idx = click_evt["rowIndex"]
        field_active = s_df.iloc[row_idx]["Field Name"]
    else:
        field_active = vd_rows[0]["field_name"] if vd_rows else None

    # ── C. Build outputs: filter grids & SQL logic ─────────
    if field_active:
        ...
        vd_sql = vd_filt["value_sql_logic"].iloc[0] \
                 if "value_sql_logic" in vd_filt.columns else ""
        pc_sql = pc_filt["value_sql_logic"].iloc[0] \
                 if "value_sql_logic" in pc_filt.columns else ""
    else:
        vd_filt, pc_filt = vd_all, pc_all
        vd_sql = pc_sql = ""

    # ── D. Return everything together ─────────────────────

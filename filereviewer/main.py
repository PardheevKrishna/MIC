...
class FileReportApp:
    def __init__(self, master):
        ...
        ttk.Entry(main, textvariable=self.excel_path, width= forty:=40).grid(row=0, column=1, sticky='w')
        ...
        self.person_cb = ttk.Combobox(main, textvariable=self.person_var, state='readonly', width= forty)
        ...
        ttk.Entry(main, textvariable=self.email_var, width= forty).grid(row=3, column=1, columnspan=2, sticky='w')
    ...
    def _generate_and_send(self):
        ...
        out_df.to_excel(report_fn, index=False)
        ...
        if win32:
            try:
                ...
                mail.Send()
                self.status.config(text=f"Report saved as {report_fn} and emailed to {email}")
            except Exception as e:
                messagebox.showerror('Email Error', f'Failed to send email:\n{e}')
                self.status.config(text=f"Report saved as {report_fn}")
        else:

...
def run_web_scraper(year, month):
    """Run web scraper for specified month and year"""
    try:
        cmd = [sys.executable, 'web_scraper.py', '--year', str(year), '--month', str(month)]
        result = subprocess.run(cmd, capture_output=True, text=True, cwd=BASE_DIR)
        return result.returncode == 0, result.stdout, result.stderr
    except Exception as e:
        return False, "", str(e)

def run_extraction(year, month):
    """Run red flag extraction for specified month and year"""
    try:
        cmd = [sys.executable, 'extract_red_flags.py', '--year', str(year), '--month', str(month)]
        result = subprocess.run(cmd, capture_output=True, text=True, cwd=BASE_DIR)
        return result.returncode == 0, result.stdout, result.stderr
    except Exception as e:
        return False, "", str(e)
...
def main():
    ...
    st.markdown('<h3 style="color: #333333; font-weight: 700; margin-bottom: 0.5rem;">Select Time Period</h3>', unsafe_allow_html=True)
    ...
    with col3:
        st.markdown('<h3 style="color: #333333; font-weight: 700; margin-bottom: 0.5rem;">Status</h3>', unsafe_allow_html=True)
    ...
    if start_button:
        ...
        with progress_container:
            ...
            sources = ['AMF', 'DFS', 'FCA', 'FED', 'FinCEN', 'FINTRAC', 'NCA', 'OFAC', 'SEC']
            
            for source in sources:
                status_placeholder.markdown(f"""
                    <div style="background: #FFF5F5; padding: 0.5rem 1rem; border-radius: 5px; margin: 0.25rem 0; border-left: 3px solid #D71E28;">
                        <span style="color: #D71E28; font-weight: 700;">Processing:</span> 
                        <span style="color: #333333; font-weight: 600;">{source}</span>
                    </div>
                """, unsafe_allow_html=True)
                time.sleep(0.3)
            
            with st.spinner("Finalizing web scraping..."):
                success, stdout, stderr = run_web_scraper(selected_year, selected_month)
            ...
            if success:
                st.markdown("""
                    <div class="progress-step completed">
                        <strong style="color: #28a745; font-size: 1.1rem;">STEP 1/2: Web Scraping - COMPLETED</strong><br>
                        <span style="color: #333333;">Data collected from all regulatory sources</span>
                    </div>
                """, unsafe_allow_html=True)
                
                # Collapsible logs
                with st.expander("View Web Scraping Logs", expanded=False):
                    st.code(stdout if stdout else "No output", language="text")
            else:
                st.markdown("""
                    <div class="error-banner">
                        <strong style="color: white; font-size: 1.2rem;">ERROR: Web scraping failed</strong><br>
                        <span style="color: white;">Please check logs below</span>
                    </div>
                """, unsafe_allow_html=True)
            ...
            extraction_status.markdown("""
                <div style="background: #FFF5F5; padding: 0.8rem 1rem; border-radius: 5px; margin: 0.5rem 0; border-left: 3px solid #D71E28;">
                    <span style="color: #D71E28; font-weight: 700;">Running:</span> 
                    <span style="color: #333333; font-weight: 600;">Gemini Multi-Model Ensemble (Flash-Lite, Flash, Pro)</span>
                </div>
            """, unsafe_allow_html=True)
            
            with st.spinner("AI models analyzing publications..."):
                success, stdout, stderr = run_extraction(selected_year, selected_month)
            ...
            if success:
                st.markdown("""
                    <div class="progress-step completed">
                        <strong style="color: #28a745; font-size: 1.1rem;">STEP 2/2: AI Analysis - COMPLETED</strong><br>
                        <span style="color: #333333;">Red flags extracted and analyzed</span>
                    </div>
                """, unsafe_allow_html=True)
                
                # Collapsible logs
                with st.expander("View Extraction Logs", expanded=False):
                    st.code(stdout if stdout else "No output", language="text")
                ...
                if df is not None:
                    ...
                    st.markdown("""
                        <div class="success-banner">
                            <strong style="color: white; font-size: 1.3rem;">ANALYSIS COMPLETED SUCCESSFULLY</strong><br>
                            <span style="color: white;">Results are ready for review and download</span>
                        </div>
                    """, unsafe_allow_html=True)
                else:
                ...
                st.markdown("""
                    <div class="error-banner">
                        <strong style="color: white; font-size: 1.2rem;">ERROR: Red flag extraction failed</strong><br>
                        <span style="color: white;">Please check logs below</span>
                    </div>
                """, unsafe_allow_html=True)
                with st.expander("Error Logs", expanded=True):
    ...
    if st.session_state.completed and st.session_state.results_df is not None:
        ...
        if 'Coverage Status' in df.columns:
            ...
            with col1:
                ...
                st.markdown(f"""
                    <div class="info-box">
                        <strong style="color: #333333; font-size: 1.1rem;">Fully Covered:</strong> 
                        <span style="color: #D71E28; font-size: 1.3rem; font-weight: 700;">{fully}</span><br>
                        <small style="color: #666666;">Matches existing red flag patterns</small>
                    </div>
                """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                st.markdown(f"""
                    <div class="info-box">
                        <strong style="color: #333333; font-size: 1.1rem;">Partially Covered:</strong> 
                        <span style="color: #D71E28; font-size: 1.3rem; font-weight: 700;">{partially}</span><br>
                        <small style="color: #666666;">Similar to existing patterns</small>
                    </div>
                """, unsafe_allow_html=True)
            ...
            with col3:
                ...
                st.markdown(f"""
                    <div class="info-box">
                        <strong style="color: #333333; font-size: 1.1rem;">Not Covered:</strong> 
                        <span style="color: #D71E28; font-size: 1.3rem; font-weight: 700;">{not_cov}</span><br>
                        <small style="color: #666666;">New red flag patterns detected</small>
                    </div>
                """, unsafe_allow_html=True)
